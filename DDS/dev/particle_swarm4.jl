...
function pso(f::F, n::Int, bounds::Vector{Float64}, num_particles::Int, num_iterations::Int, w::Float64, c1::Float64, c2::Float64, tol::Float64) where {F<:Function}
    a, b = bounds
    positions = Vector{SVector{n, Float64}}(undef, num_particles)
    velocities = Vector{SVector{n, Float64}}(undef, num_particles)
    best_positions = Vector{SVector{n, Float64}}(undef, num_particles)
    best_scores = Vector{Float64}(undef, num_particles)

    for i = 1:num_particles
        positions[i] = @SVector rand(Uniform(a, b), n)
        velocities[i] = @SVector rand(Uniform(a, b), n)
...
begin
    logistic_dim = 1
    logistic2(x::SVector{logistic_dim, Float64}, p) = @SVector [p[1]*x[1]*(1-x[1])]
    param = [3.235]
    n = 2
