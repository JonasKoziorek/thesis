...
function pso(f::F, n::Int, bounds::Vector{Float64}, num_particles::Int, num_iterations::Int, w::Float64, c1::Float64, c2::Float64, tol::Float64) where {F<:Function}
    a, b = bounds
    positions = Vector{Vector{Float64}}(undef, num_particles)
    velocities = Vector{Vector{Float64}}(undef, num_particles)
    best_positions = Vector{Vector{Float64}}(undef, num_particles)
    best_scores = Vector{Float64}(undef, num_particles)
    for i = 1:num_particles
        positions[i] = rand(Uniform(a, b), n)
        velocities[i] = zeros(n)
        best_positions[i] = rand(Uniform(a, b), n)
        best_scores[i] = Inf
    end
