...
function F_pp(func::F, z_p, λ) where {F<:Function}
    ...
    func2 = DDS.nth_composition(logistic2, 12)
    res = func2(z_p, λ)
    # res = MVector(λ[1]*z_p[1]*(1-z_p[1]))
    res=norm(res.-z_p)
    return res
end
...
function test3()
    ...
    param = [3.235]
    nth_logistic = DDS.nth_composition(logistic2, n)
    objective_func(x) = F_pp(nth_logistic, x, param)
    x_range = [0.0, 1.0]
    num_particles = 30
    num_iterations = 300
    w = 0.729
    ...
    tol = 1e-5
    dim = 1
    @time x0, _ = pso(objective_func, dim, x_range, num_particles, num_iterations, w, c1, c2, tol)
end
test3()
end
