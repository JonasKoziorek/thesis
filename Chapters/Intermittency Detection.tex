...
\textcolor{blue}{
Global Search identified intervals containing a breakpoint.
Naive Local Search (NLS) uses the these intervals to find precise locations of the breakpoints.
...
If $p_{C}$ is non-periodic then it is known that the breakpoint is in the interval $[p_{C}, p_{B}]$.
...
Pseudocode for the NLS of is given in Algorithm~\ref{alg:local_search}.
}
