...
Lastly, the ambiguity of the bifurcation diagram caused by intermittency is shown.
}
...
% \textcolor{green}{
% To verify whether the characteristic relation holds, the following numerical experiment can be conducted.
% Logistic map $\mathcal{L}_{r}$. For $r^{*}=1 + \sqrt{8}$ a saddle-node bifurcation occurs.
% There are three fixed points of $\mathcal{L}_{r^{*}}$ at which the derivative of the map is equal to $1$.
% These three fixed points are $x^{*}_{1} \approx 0.1599$, $x^{*}_{2} \approx 0.5143$ and $x^{*}_{3} \approx 0.9563$.
% For parameter $r_{i}$ slightly less than $r^{*}$ a narrow channel is formed between the graph of $\mathcal{L}_{r}$ and identity at in the proximity of these three fixed points.
% Select one of the fixed points, for instance $x^{*}_{1}$.
% Furthermore select a laminar interval around it, for instance $I_{l} = [x^{*}_{1}-c, x^{*}_{1}+c]$ for $c = 0.03$.
% Next, select some $n$ parameters less than $r^{*}$.
% For each parameter map $\mathcal{L}_{r_{i}} \forall i \in [1, n]$ generate $m$ point reinjected into $I_{l}$.
% For each reinjected point calculate its laminar length using \eqref{eq:laminar_length}.
% Calculate average of laminar lengths for each corresponding parameter $r_{i}$.
% Now, each of the $n$ parameters $r_{i}$ have a corresponding $l_{avg}$.
% Finally, compute $\epsilon$
% }
...
If the trajectory constructed for iterations $n \in [n_{total}, n_{total}-n_{last}]$ lies fully in the laminar phase, then only a finite number of dots will be displayed on the diagram.
...
This might not be a big deal for individual parameters $p$, but this ambiguity might affect a wide range of parameters $p$ near the breakpoint.
}
\par
\textcolor{blue}{
