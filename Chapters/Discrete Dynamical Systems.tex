...
\begin{definition}[Periodic point]
    Let $\left( X, f\right)$ be a DDS. The point $x \in X$ is called \emph{$n$-periodic} if $f^{n}(x)=x$ for any $n \in \mathbb{N}$.
\end{definition}
...
\begin{definition}[Eventually periodic point]
    ...
    Hence, $f^{i}(x)$ is $n$-periodic for all $i \geq m$.~\cite{Devaney20211026}
    }
\end{definition}
...
\textcolor{darkred}{
    ...
    Then the set $\{f^{n+m}(x_0):m \in \mathbb{N}\}$ is called an \emph{$n$-periodic orbit}.
    The set of all attracting $n$-periodic points of $\left( X, f\right)$ is called \emph{stable periodic orbit} (abbr. SPO).
    The set of all periodic $n$-periodic points of $\left( X, f\right)$ is called \emph{unstable $n$-periodic orbit} (abbr. UPO).
    % The set of all stable periodic points
