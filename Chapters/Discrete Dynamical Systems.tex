...
\begin{definition}[Discrete dynamical system]
    A \emph{discrete dynamical system} (abbr. DDS) is a tuple $\left( X, f \right)$ where X is a non-empty set and $f: X \rightarrow X$ is a map~\emph{\cite{Brin20100706}}.
...
\begin{remark}[Logistic map]
    ...
    He proposed that student should be introduced to Logistic map early on in their studies to understand that simple rules can yield complex behavior.
    \par
...
\label{def:fixed point}
    \textcolor{darkred}{
    Let $\left( X, f \right)$ be a DDS. Point $x \in X$ is called a \emph{fixed point} of $f$ if and only if $f(x) = x$.
    }
\end{definition}
...
\begin{remark}
    ...
    Consequently, point $x=0$ becomes repelling and an attractive fixed point $x=(r-1)/r$ is created.
    }
    \par
    \textcolor{darkred}{
...
\textcolor{darkred}{
    Let $\left( X, f_{p}\right)$ be a DDS.
    Let $x_0$ be an $n$-periodic point.
    Then the set $O:=\{f_{p}^{n+m}(x_0):m \in \mathbb{N}\}$ is called an \emph{$n$-periodic orbit}.
    If $x_{0}$ is attracting, then the set $O$ is called \emph{stable periodic orbit} (abbr. SPO).
    The set of all $n$-periodic points of $f_{p}$ is called \emph{unstable $n$-periodic orbit} (abbr. UPO).
}
\end{definition}
...
\label{def:bifurcation}
    ...
    A \emph{bifurcation} occurs when the qualitative behavior of $f_p$ changes as the parameter $p$ slightly changes.
\end{definition}
...
\begin{remark}
    ...
    This type of bifurcation is called saddle-node bifurcation.
    ...
    This type of bifurcation is called period-doubling bifurcation.
\end{remark}
